...
class Patient(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "acuity_level >= 1 AND acuity_level <= 5", name="check_acuity_level_range"
        ),
    )
...
class VitalSigns(Base):
    ...
    heart_rate: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Heart rate in beats per minute (30-200 bpm)"
    )

    systolic_bp: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Systolic blood pressure in mmHg (50-300 mmHg)"
    )

    diastolic_bp: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Diastolic blood pressure in mmHg (20-200 mmHg)"
    )
    ...
    oxygen_saturation: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Oxygen saturation percentage (50-100%)"
    )

    temperature: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Body temperature in Celsius (30-45°C)"
    )
    ...
    recorded_by: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="Who recorded these vital signs (optional)"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "heart_rate >= 30 AND heart_rate <= 200", name="check_heart_rate_range"
        ),
        CheckConstraint(
            "systolic_bp >= 50 AND systolic_bp <= 300", name="check_systolic_bp_range"
        ),
        CheckConstraint(
            "diastolic_bp >= 20 AND diastolic_bp <= 200",
            name="check_diastolic_bp_range",
        ),
        CheckConstraint("diastolic_bp < systolic_bp", name="check_bp_relationship"),
        CheckConstraint(
        ...
        CheckConstraint(
            "temperature >= 30 AND temperature <= 45", name="check_temperature_range"
        ),
...
class RiskAssessment(Base):
    ...
    risk_score: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Numerical risk score between 0 and 1"
    )

    risk_flag: Mapped[bool] = mapped_column(
        Boolean, nullable=False, comment="Boolean indicator of high deterioration risk"
    )
    ...
    model_version: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="Version of the ML model used for assessment"
    )
    ...
    patient: Mapped["Patient"] = relationship(
        "Patient", back_populates="risk_assessments"
    )

    vital_signs: Mapped["VitalSigns"] = relationship(
        "VitalSigns", back_populates="risk_assessments"
    )

    # Constraints
    __table_args__ = (
        CheckConstraint(
            "risk_score >= 0.0 AND risk_score <= 1.0", name="check_risk_score_range"
        ),
        # Unique constraint to prevent duplicate assessments for same vital signs
        UniqueConstraint("vital_signs_id", name="unique_assessment_per_vital_signs"),
