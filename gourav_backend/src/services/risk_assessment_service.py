...
class RiskAssessmentService:
    ...
    def __init__(self, db: Session, model_endpoint: Optional[str] = None,
                 model_timeout: int = 5, model_version: str = "v1.0.0"):
        """
        Initialize service with database session and ML model configuration.
        
        Args:
            db: SQLAlchemy database session
            model_endpoint: ML model endpoint URL (None for mock mode)
            model_timeout: ML model request timeout in seconds (default 5 per Requirement 3.5)
        ...
        # Ensure timeout doesn't exceed maximum allowed (Requirement 3.5)
        effective_timeout = min(model_timeout, self.MAX_ASSESSMENT_TIME_SECONDS)
        
        # Initialize ML model client
        self.ml_client = RiskModelClient(
    ...
    def _perform_risk_assessment(self, patient: Patient, vital_signs: VitalSigns) -> RiskAssessment:
        ...
        try:
            ...
            risk_score, risk_flag, processing_time_ms = self.ml_client.predict_risk(
                **model_input
            )
    ...
    def _fallback_risk_assessment(self, patient: Patient, vital_signs: VitalSigns) -> tuple[float, bool]:
        ...
        risk_score += acuity_risk.get(patient.acuity_level, 0.5)
        ...
        # Risk flag if score > 0.5 or acuity >= 4
        risk_flag = risk_score > 0.5 or patient.acuity_level >= 4
